    ...
    return (
        <div className="flex items-center justify-center mx-auto w-full mb-[100px] max-w-[var(--section-max-width)] px-[var(--section-padding-x-sm)] md:px-[var(--section-padding-x)] flex-col gap-[var(--testimonials-outer-gap)]">
            <div className="flex flex-col justify-center space-y-6">
                ...
                {/* blog post cards — snap scroll on mobile, row on desktop */}
                <div className="flex flex-row gap-6 h-fit
                    overflow-x-auto snap-x snap-mandatory
                    -mx-[var(--section-padding-x-sm)] px-[var(--section-padding-x-sm)]
                    ...
                    {INSIGHTS_BLOG_POSTS.map((post, i) => (
                        <div
                            ...
                            className="rounded-xl overflow-hidden
                                shrink-0 min-w-[82vw] snap-center
                                md:shrink md:min-w-0 md:w-1/3
                                flex flex-col space-y-3 md:space-y-4
                                group cursor-pointer transition-all ease-in-out duration-200"
                        >
                            ...
                            <h4 className="text-base md:text-[20px] font-medium font-outfit group-hover:text-[#E5383B]">
                                {post.title}
                            </h4>
                            <p className="font-outfit text-sm md:text-[18px] text-[#696366]">
                                {post.description}
                            </p>
                        </div>
                    ))}
                ...
                <div className="mt-6 font-outfit flex flex-row gap-4 group">
                    <button className="group-hover:text-[#E5383B] transition-all ease-in-out duration-200 cursor-pointer text-sm md:text-base">
                        Explore more blogs
                    </button>
                    <div className="items-center justify-center flex">
                        <img src="src\assets\insights\arrow icon.png" className="w-fit h-fit opacity-0 group-hover:opacity-100 transition-all ease-in-out duration-200 -translate-x-5 group-hover:translate-x-0" alt="" />
                    </div>
                </div>
            </div>
        </div>
